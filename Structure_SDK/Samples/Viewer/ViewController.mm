...

{
    ...
    if(![_socket connectToHost:@"192.168.191.1" onPort:4000 error:&err])
    ...
    // [_socket writeData:[@"jjjjjj" dataUsingEncoding:NSUTF8StringEncoding] withTimeout:-1 tag:0];
    ...
    _normalsImageView.contentMode = UIViewContentModeScaleAspectFit;
     _normalsImageView.userInteractionEnabled = YES;
    [self.view addSubview:_normalsImageView];
    
    UITapGestureRecognizer *doubleTapGestureRecognizer = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(doubleTap:)];
    [doubleTapGestureRecognizer setNumberOfTapsRequired:2];
    [_normalsImageView addGestureRecognizer:doubleTapGestureRecognizer];
...
- (void)doubleTap:(UIGestureRecognizer*)sender
{
   CGPoint point = [sender locationInView:self.view];
    tem1 = (int)(10000*point.x/self.view.frame.size.width);
    tem2 = (int)(10000*point.y/self.view.frame.size.height);
}
...
{
   // [_socket readDataWithTimeout:-1 tag:111];
...

{
    ...
    [ardata appendData:data];

    readlen = readlen - data.length;
    if(readlen>0)
    [_socket readDataToLength:readlen withTimeout:-1 tag:0];
    
  // int a = readlen;
   // [_socket writeData:[[NSString stringWithFormat: @"%d", a] dataUsingEncoding:NSUTF8StringEncoding] withTimeout:-1 tag:0];
    pdep = true;
    pcol = true;
...

{
    ...
    size_t cols = depthFrame.width;//320
    
    size_t rows = depthFrame.height;//240
    ...
    if(depdata == NULL)
        depdata =(uint16_t*)malloc(cols * rows * 2);

    int depindex =0;
    
    if(pdep && pcol)
    {
        for(depindex = 0;depindex<320*240;depindex++)
        ...
        NSData *rawdata = [NSData dataWithBytes:depdata length:cols * rows*2];       
        [_socket writeData:rawdata withTimeout:-1 tag:0];
        pdep = false;
           }
    ...
    NSData *rawdata = [NSData dataWithBytes:depthFrame.shiftData length:cols * rows*2];
    [_socket writeData:rawdata withTimeout:-1 tag:0];
    tem1=0;
    }
    */
    ...
    /*BitmapImageRep * bitmapImageRep = [[bitmapImageRep alloc]initWithCGImage:imageRef];
    ...
    // NSData*  datapackage1 =UIImageJPEGRepresentation([UIImage imageWithCGImage:imageRef],0.5);
...

{
    ...
    size_t cols = normalsFrame.width;//320
    
    size_t rows = normalsFrame.height;//240
   /* int a = cols;
    int b = rows;
    [_socket writeData:[[NSString stringWithFormat: @"%d", a] dataUsingEncoding:NSUTF8StringEncoding] withTimeout:-1 tag:0];
    [_socket writeData:[[NSString stringWithFormat: @"%d", b] dataUsingEncoding:NSUTF8StringEncoding] withTimeout:-1 tag:0];*/
    ...
    bitmapInfo = (CGBitmapInfo)kCGImageAlphaNone;//SkipFirst;
  ...
  //  readlen=307200;
    ...
    [_socket readDataToLength:320*240*3 withTimeout:-1 tag:307222];
    
   /* for (readlen=0;readlen<307;readlen++)
    {
    
        [_socket readDataToLength:1000 withTimeout:-1 tag:0];

    }
*/
    
    
    CGDataProviderRef provider = CGDataProviderCreateWithCFData((CFDataRef)ardata);
...

{
    ...
    size_t cols = CVPixelBufferGetWidth(pixelBuffer); //640
    
    size_t rows = CVPixelBufferGetHeight(pixelBuffer); //480
    ...
    if(_colrgbbuffer==NULL)
        
        _colrgbbuffer = (uint8_t*)malloc(320*240*3);
    ...
    if(!pdep&&pcol)
    {
        ...
        int k=0;
        int l=0;
        for(k=0;k<640*480*4;k++){
            _colrgbbuffer[l++]=_colorImageBuffer[k];
            
            if (l%3==0){
                tem =  _colrgbbuffer[l-1];
                _colrgbbuffer[l-1]= _colrgbbuffer[l-3];
                _colrgbbuffer[l-3]=tem;
            }
        ...
        NSData *coldata = [NSData dataWithBytes:_colrgbbuffer length:320*240*3];
        [_socket writeData:coldata withTimeout:-1 tag:0];
        pcol = false;
...
//scall image to 240*320
    ...
    NSData *coldata = [[NSData alloc] initWithBytes:_colrgbbuffer length:320*240*3];
     
     NSTimeInterval nowtime = [[NSDate date] timeIntervalSince1970]+5.2;
      NSMutableData *coltimedata = [[NSMutableData alloc] initWithBytes:&nowtime length:8];
        [coltimedata appendData:coldata];
        
        
    [_socket writeData:coltimedata withTimeout:-1 tag:0];
        tem2=0;
    }
  ...
  //send timestep
   ...
   //    NSDate *date = [NSDate dateWithTimeIntervalSince1970:colorFrame.timestamp];
   /* NSDate *date = [NSDate date];
    NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
    [dateFormat setDateFormat:@"YYYY-MM-dd'T'HH:mm:ss.SSS"];
    NSString *_date=[dateFormat stringFromDate:date];
    [_socket writeData:[_date dataUsingEncoding:NSUTF8StringEncoding] withTimeout:-1 tag:0];
    */
